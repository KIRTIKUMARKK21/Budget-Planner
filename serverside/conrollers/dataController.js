...
export const getCurrentprice= async (req, res)=>{
  ...
  const arr = [];
  const user=await RecordModel.findById(req.params.id);
    
    try{
      for (let i = 0; i < user.stock.length; i++) {
        
        let symbol=user.stock[i].symbol;
        let stockPrice = (await fetchsymbls(symbol,"pk_6fa1ccc6513640abb6257fc93550e73b")).data[symbol].quote.latestPrice;
        let currencyRate = 77.5;
        let purchaseprice=user.stock[i].purchase
      const num=stockPrice*currencyRate
...
export const fetchsymbols = async (req, res) => {
  axios.get(`https://cloud.iexapis.com/stable/stock/market/batch?symbols=${req.params.symbols}&filter=symbol,companyName,latestPrice,latestUpdate,previousClose,lastTradeTime&types=quote&token=pk_6fa1ccc6513640abb6257fc93550e73b`)
...
axios.get(`https://cloud.iexapis.com/stable/stock/${req.params.symbols}/quote?filter=symbol,companyName,latestPrice,latestUpdate,previousClose,lastTradeTime&token=pk_6fa1ccc6513640abb6257fc93550e73b`)
...
export const getStocks=async (req,res)=>{
  const Stocks=await RecordModel.findById(req.params.id);
  res.status(200).json({
    status: 200,
    data: Stocks.stock,
  });
...
export const Addtostock = async (req, res) => {
  const { symbol ,date,purchase} = req.body;
  // add or update product in stock array
  try {
    let user = await RecordModel.findById(req.user._id);
    let flag = 0;
    if(purchase.price===0){
 //remove item with with 0 quantity
      user.stock.forEach((item, index) => {
        if (item.symbol.toString() === symbol) {
          user.stock.splice(index, 1);
          flag = 1;
        }
      });
    }for (let i = 0; i < user.stock.length; i++) {
      for(let j = 0; j < user.stock.price.length; j++)
      if (user.stock[i].symbol.toString() === symbol && user.stock[j].purchase.price==purchase.price) {
        user.stock[i].purchase.price = purchase.price;
        flag = 1;
      }
    }
    if (flag === 0) {
